...
/** Vista paginada solo para Cabezal (Carreta | Material) */
export default function CabezalCatalogDialog({ open, onOpenChange }) {
  const TAKE = 12;

  const [categoria, setCategoria] = useState("carreta");     // "carreta" | "material"
  ...
  const listParams = useMemo(() => {
    ...
    if (categoria === "material") p.materialTipo = materialTipo;
    if (debouncedQ?.trim()) p.q = debouncedQ.trim();
    p.skip = skip;
    p.take = TAKE;
    return p;
  }, [categoria, materialTipo, debouncedQ, skip]);
  ...
  async function load() {
    ...
    try {
      ...
      console.error("[CabezalCatalogDialog] list error:", e?.response?.data || e);
      setRows([]);
      setTotal(0);
  ...
  // al cambiar de pestaña, ir a página 1
  useEffect(() => { setSkip(0); }, [categoria, materialTipo, debouncedQ]);
  ...
  async function saveEdit() {
    ...
    await trailersService.update(editingId, {
      ...
      materialTipo: edit.categoria === "material" ? edit.materialTipo : undefined,
    });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-6xl p-0 overflow-hidden">
        <DialogHeader className="px-6 pt-6">
          <DialogTitle className="text-xl">Cabezal — Catálogo de placas</DialogTitle>
          <DialogDescription>Gestiona placas por subcategoría con una vista más limpia y paginada.</DialogDescription>
        </DialogHeader>
        ...
        <div className="px-6 pb-6">
          <Tabs value={categoria} onValueChange={setCategoria} className="space-y-4">
            <TabsList className="grid w-full grid-cols-2 rounded-2xl bg-gray-100 p-1">
              <TabsTrigger value="carreta" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow">
                Carreta
              </TabsTrigger>
              <TabsTrigger value="material" className="rounded-xl data-[state=active]:bg-white data-[state=active]:shadow">
                Material
              </TabsTrigger>
            </TabsList>
              ...
              <div className="lg:col-span-8 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    {loading ? "Cargando…" : `Mostrando página ${page} de ${totalPages} (${total} total)`}
                  </div>
